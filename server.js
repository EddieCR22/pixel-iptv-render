...
async function startAudioAttempt(req, target, profile) {
  let r;
  ...
  const proc = spawn(ffmpegPath, audioFixArgs(), { stdio:["pipe","pipe","pipe"] });
  let stderr = "";
...
async function handleLiveAudioFix(req, res, session, id) {
  ...
  try {
    for (const target of candidates) {
      for (const profile of profiles) {
        const attempt = await startAudioAttempt(req, target, profile);
        ...
        console.log(`[audiofix-v013] start id=${id} profile=${profile.id}`);
        return;
      }
    }
    release();
    console.warn(`[audiofix-v013] failed id=${id} ${failures.join(" | ").slice(-1500)}`);
    return sendText(res, "No se pudo obtener audio compatible desde el MPEG-TS original.", 502, {"X-Pixel-Stream-Mode":"audiofix"});
  } catch (e) {
    release();
    throw e;
  }
...
async function handleStream(req, res, url, kind, id) {
  ...
  if (kind === "live" && mode === "audiofix") return await handleLiveAudioFix(req, res, s, id);
